...
page.on("response", (r) => {
  const u = r.url();
  if (/pacientes|anamnese|prontuario|odontograma/.test(u)) errs.push("[RESP] " + r.status() + " " + u.slice(0, 120));
});
...
console.log("TEM 'Dados cadastrais':", body.includes("Dados cadastrais"));
console.log("TEM 'Ficha & Avaliação':", body.includes("Ficha & Avaliação"));
console.log("URL:", await page.evaluate(() => location.pathname + location.search));
console.log("\nTRACE/RESOURCES/ERROS:\n" + (errs.slice(-20).join("\n") || "nenhum"));
await browser.close();