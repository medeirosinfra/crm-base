...
import { useClinicNav } from "@/hooks/use-clinic-nav";

export function ClinicSidebar() {
  const { state } = useSidebar();
  const navigate = useNavigate();
  const { signOut, user, tenantNome, segmento } = useAuth();
  const { items } = useClinicNav();
  ...
  const principais = items.filter((i) => i.grupo === "principal");
  const config = items.filter((i) => i.grupo === "configuracao");
